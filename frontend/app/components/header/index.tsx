...
import { ContextUser, UserContext } from "@/app/providers/user-provider";
...
export default function Header() {
    ...
    const  profile = (name: string) =>{
        ...
        return `${profileName?.[0]?.charAt(0).toUpperCase() || ""} ${profileName?.[1]?.charAt(0).toUpperCase() || ""}`;
    }
        ...
        <header className="w-full shadow-sm  inset-x-0 top-0 z-50 sticky bg-white">
            <Container className="h-16 flex items-center justify-between py-2 ">
                ...
                <div className="flex items-center justify-center gap-4 bg-gray-200/40 px-2 py-1 rounded-full">
                    <h1 className="text-xl text-center font-semibold text-black ml-2">{profile(user?.name || "") || "acessar conta >"}</h1>
                    <DropdownMenu>
